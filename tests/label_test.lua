...
h.describe("label font roles + reskin ownership", function()
  h.it("section uses the overline role (11px Medium); paragraph uses body (LineHeight)", function()
    local sec = Label.new({ Text = "General", Variant = "section" })
    h.expect(sec.Frame.TextSize).toBe(R.Theme.Font.overline.Size)
    h.expect(sec.Frame.FontFace.Weight).toBe(h.roblox.Enum.FontWeight.Medium)
    h.expect(sec.Frame.Font).toBe(h.roblox.Enum.Font.BuilderSans)
    local par = Label.new({ Text = "long text", Variant = "paragraph" })
    h.expect(par.Frame.TextSize).toBe(R.Theme.Font.body.Size)
    h.expect(par.Frame.LineHeight).toBe(R.Theme.Font.body.LineHeight)
    local def = Label.new({ Text = "x" })
    h.expect(def.Frame.TextSize).toBe(R.Theme.Font.body.Size)
  end)
  h.it("Destroy unregisters the AccentReg closure (no leaked reskin closure)", function()
    local unregs, fns = 0, {}
    local reg = function(fn) fns[#fns + 1] = fn; return function() unregs = unregs + 1 end end
    local l = Label.new({ Text = "x", AccentReg = reg })
    h.expect(#fns).toBe(1)
